...
def load_ret(stocks, start_date, end_date):
    px = load_px(stocks, start_date, end_date).dropna()
    return px.pct_change()


def load_corr(stocks, start_date, end_date, window=42):
    ret = load_ret(stocks, start_date, end_date)
    corr = ret.rolling(window=window, min_periods=window).corr(ret['SPY']).resample('B').first().ffill().dropna().drop(
        columns='SPY')
    return corr
